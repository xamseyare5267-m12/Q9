...
function startServer() {
  ...
  const upload = multer({
    ...
    fileFilter: (req, file, cb) => {
      ...
      const allowedExtensions = ['.pdf', '.docx', '.jpg', '.jpeg', '.png', '.webp', '.gif', '.mp4'];
      const ext = path.extname(file.originalname).toLowerCase();
      
      if (allowedExtensions.includes(ext)) {
        cb(null, true);
      } else {
        cb(new Error(`File type ${ext} not supported. Permitted types: PDF, DOCX, JPG, JPEG, PNG, WEBP, GIF, MP4.`));
      }
    }
  });
...
Thank you for choosing SomLuul!`;
  ...
  app.post('/api/owner/auth/validate', (req, res) => {
    ...
    if (ownerBruteForce.lockUntil > Date.now()) {
    ...
    // Username: MXDdeeq207, Password: 615599869612912055
    if (username === 'MXDdeeq207' && password === '615599869612912055') {
      ...
      ownerBruteForce.failedAttempts += 1;
