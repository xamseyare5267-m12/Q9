 ...
 * Returns a controller object to stop the ringtone when answered/ended.
 */
  ...
  const playRingCycle = () => {
    ...
    try {
      ...
      const createTone = (freq: number, startTime: number, duration: number) => {
        ...
        // Gentle attack/decay envelope to avoid clicking
      ...
      // Ring pulse 1: 0.8s
      createTone(440, now, 0.8);
      createTone(480, now, 0.8);

      // Brief gap: 0.2s, then Ring pulse 2: 0.8s
      createTone(440, now + 1.0, 0.8);
      createTone(480, now + 1.0, 0.8);
    } catch (e) {
