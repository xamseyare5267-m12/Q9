...
export const MessengerSection: React.FC<MessengerSectionProps> = ({ user, authToken, onShowToast, onNavigateHome }) => {
  ...
  const renderAvatar = (avatarUrl: string | null | undefined, name: string, sizeClass = "w-10 h-10") => {
    const isValidUrl = avatarUrl && (avatarUrl.startsWith('http') || avatarUrl.startsWith('data:image') || avatarUrl.startsWith('/')) && !avatarUrl.includes('unsplash.com');
    if (isValidUrl) {
      return (
        <img 
          src={avatarUrl} 
  ...
  return (
    <DeviceFrame language={language}>
      ...
      <div id="messenger-wrapper" className="bg-white dark:bg-[#141b2d] rounded-2xl shadow-sm h-[75vh] overflow-hidden grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 relative border border-gray-150 dark:border-gray-800/60 animate-fade-in">
        ...
        <div className={`border-r border-gray-150 dark:border-gray-800/60 flex flex-col h-full bg-gray-50/50 dark:bg-[#121826] md:col-span-1 pt-3 ${mobileView === 'chat' ? 'hidden md:flex' : 'flex'}`}>
          ...
          <div className="px-4 py-3 bg-white dark:bg-[#141b2d] flex justify-between items-center select-none border-b border-gray-100 dark:border-gray-850">
            <button
              type="button"
              onClick={() => {
                if (onNavigateHome) onNavigateHome();
              }}
              className="text-xl font-black text-blue-600 dark:text-blue-500 tracking-tight font-sans flex items-center gap-2 hover:opacity-80 transition-opacity cursor-pointer text-left"
        ...
        <div className={`flex flex-col h-full bg-white dark:bg-[#141b2d] ${showContactInfo ? 'md:col-span-2 lg:col-span-2' : 'md:col-span-2 lg:col-span-3'} pt-6 ${mobileView === 'list' ? 'hidden md:flex' : 'flex'}`}>
          
          {activeRoom ? (
            <>
              {/* Active room headers */}
              <div className="p-3 border-b border-gray-100 dark:border-gray-800/60 flex justify-between items-center bg-gray-50/20 dark:bg-gray-900/10">
              ...
              <div ref={chatContainerRef} className="grow overflow-y-auto p-4 space-y-4 bg-gray-50/20 dark:bg-[#0b0f19]/15">
              ...
              <div className="p-3 border-t border-gray-100 dark:border-gray-800/60 bg-gray-50/10 dark:bg-gray-900/10">
                ...
                >
                  <input
                    ...
                    placeholder={isCurrentRoomBlocked ? 'You have blocked this contact. Unblock to message.' : (activeRoom.isSecret ? t('secret_chat') : t('type_message'))}
                    className="grow bg-gray-50 dark:bg-[#1f293d] border border-gray-150 dark:border-gray-700 rounded-full px-4 py-2 text-xs text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none"
                    value={inputText}
                    onChange={(e) => setInputText(e.target.value)}
                  />

                  {/* Real Voice Note Recorder controls */}
                  {(audioRecorder.isRecording || isRecording) ? (
                    <div className="flex items-center gap-2 bg-red-500/10 border border-red-500/20 px-3 py-1.5 rounded-full text-red-500 text-xs font-mono font-bold animate-pulse">
                      <span className="w-2 h-2 rounded-full bg-red-500 animate-ping shrink-0" />
                      <span>0:{((audioRecorder.recordingSeconds || recordingSeconds) < 10 ? '0' : '')}{(audioRecorder.recordingSeconds || recordingSeconds)}</span>
                      
                      <button 
                        type="button" 
                        onClick={handleCancelVoiceRecording} 
                        className="bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-200 rounded-full p-1 hover:bg-gray-300 dark:hover:bg-gray-600 cursor-pointer ml-1"
                      ...
                      <button 
                        ...
                        className="bg-red-600 text-white rounded-full p-1 hover:bg-red-700 cursor-pointer"
            ...
            <div className="grow flex flex-col items-center justify-center text-center p-6 bg-gray-50/20 dark:bg-[#0b0f19]/10">
