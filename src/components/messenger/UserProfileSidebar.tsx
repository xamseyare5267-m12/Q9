...
import { 
  ...
  FolderMinus, Info, Phone, BadgeAlert, HelpCircle, Flame
} from 'lucide-react';
...
export const UserProfileSidebar: React.FC<UserProfileSidebarProps> = ({
  ...
  const emailPrefix = matchingRealProfile?.email ? `@${matchingRealProfile.email.split('@')[0]}` : '@user';
  const displayEmail = matchingRealProfile?.email || 'somluul_user@somluul.com';
  ...
  const renderSidebarAvatar = (avatarUrl: string | null | undefined, name: string) => {
    ...
    return (
      <div className="w-24 h-24 rounded-full bg-gradient-to-tr from-blue-600 via-indigo-600 to-purple-600 text-white flex items-center justify-center font-black text-2xl border-4 border-white dark:border-gray-900 shadow-md tracking-tight font-sans">
        {initials}
      </div>
    );
  };
  ...
  return (
    <div className="w-full h-full flex flex-col bg-gray-50/50 dark:bg-[#111624] overflow-y-auto scrollbar-thin">
      {/* Header section */}
      <div className="flex justify-between items-center p-4 border-b border-gray-150 dark:border-gray-800/60 bg-white dark:bg-[#141b2d]">
      ...
      <div className="relative pb-4 border-b border-gray-150 dark:border-gray-800/60 bg-white dark:bg-[#141b2d] space-y-3">
        ...
        <div className="flex flex-col items-center text-center -mt-14 px-4 relative z-10 space-y-2">
          ...
          <div>
            <h3 className="font-bold text-base text-gray-955 dark:text-white flex items-center justify-center gap-1.5">
              {room.name}
              {room.id === 'r1' && (
