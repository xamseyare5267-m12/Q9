...
export const VoiceNotePlayer: React.FC<VoiceNotePlayerProps> = ({ mediaUrl, durationLabel, isMe }) => {
  ...
  useEffect(() => {
    if (mediaUrl && (mediaUrl.startsWith('data:audio') || mediaUrl.startsWith('http') || mediaUrl.startsWith('blob:'))) {
    ...
    if (isPlaying) {
      ...
      if (mediaUrl && (mediaUrl.startsWith('data:audio') || mediaUrl.startsWith('blob:') || mediaUrl.startsWith('http') || mediaUrl.startsWith('/'))) {
        ...
        try {
          ...
          console.warn('Native HTML5 Audio play notice:', err);
          // If native play fails, simulate progress without music synth noise
  ...
  return (
    <div className={`p-2.5 rounded-2xl flex items-center gap-3 max-w-xs sm:max-w-sm ${isMe ? 'bg-blue-600 text-white' : 'bg-white dark:bg-gray-800 text-gray-900 dark:text-white border border-gray-150 dark:border-gray-700/80 shadow-xs'}`}>
      {/* Play/Pause Circle */}
      <button
        onClick={togglePlay}
        type="button"
