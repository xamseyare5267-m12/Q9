...
export const ProfileSection: React.FC<ProfileSectionProps> = ({
  ...
  const renderAvatarBubble = (avatarUrl: string | null | undefined, name: string, sizeClass: string = "w-24 h-24") => {
    const isUrl = avatarUrl && (avatarUrl.startsWith('http') || avatarUrl.startsWith('/') || avatarUrl.startsWith('data:image'));
    if (isUrl) {
      return (
        <img
          src={avatarUrl}
    ...
    if (parts.length >= 2) {
      ...
      initials = user?.first_name ? `${user.first_name[0]}${user.last_name ? user.last_name[0] : ''}`.toUpperCase() : 'QY';
    }
            ...
            {isOwnProfile ? (
              <>
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-gray-100 hover:bg-gray-200 text-gray-800 dark:bg-gray-800 dark:hover:bg-gray-700 dark:text-white text-xs font-bold rounded-xl transition-all flex items-center gap-1.5"
                >
                  <Edit3 size={15} />
              ...
              <>
                ...
                <button
                  ...
                  className={`px-5 py-2.5 text-xs font-bold rounded-xl transition-all shadow-sm flex items-center gap-1.5 ${
                ...
                <button
                  ...
                  className="px-4 py-2.5 text-xs font-bold bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
                >
                ...
                <button
                  ...
                  className="px-4 py-2.5 text-xs font-bold bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl shadow-xs transition-all flex items-center gap-1.5 cursor-pointer active:scale-95"
              ...
              {photosUploaded.length === 0 ? (
                ...
                <div className="grid grid-cols-3 gap-1.5 rounded-xl overflow-hidden">
                  {photosUploaded.slice(0, 9).map((url, idx) => (
                    <div key={idx} className="aspect-square bg-gray-50 overflow-hidden hover:opacity-90 transition-all cursor-pointer">
                ...
                {postInputs.mediaUrl && (
                  <div className="relative rounded-xl overflow-hidden border bg-black h-48 flex items-center justify-center">
                    {postInputs.mediaType === 'image' ? (
                      ...
                      <video src={postInputs.mediaUrl} controls className="h-full" />
                    )}
                    <button
                      type="button"
                        ...
                        if (isVideoUrl && p.mediaUrl) {
                          return (
                            <div className="rounded-xl overflow-hidden bg-black max-h-[350px] flex items-center justify-center">
                              <video src={p.mediaUrl} controls className="w-full max-h-[350px]" playsInline preload="metadata" />
                            </div>
                          );
                        }
                        if (p.mediaUrl) {
                          return (
                            <div className="rounded-xl overflow-hidden bg-slate-50 max-h-[350px]">
