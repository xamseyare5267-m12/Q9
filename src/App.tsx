...
const getApiBaseUrl = () => {
  ...
  // Otherwise, if running inside a compiled Android app (e.g. file: protocol or Capacitor/Cordova webview)
  // or in an offline environment, fall back to the absolute production server
        ...
        if (err.response && (err.response.status === 401 || err.response.status === 403)) {
          ...
          try {
            ...
            if (backupProfileStr) {
              ...
              if (retryResponse.data.user.email.toLowerCase() === 'xamseyare5267@gmail.com') {
                setActiveTab('admin_center');
              } else {
                setActiveTab('feed');
              }
          ...
          // Network / offline error - keep the local session as is so they don't get logged out
          try {
            const parsed: AuthSession = JSON.parse(saved);
            setSession(parsed);
          } catch (_) {}
